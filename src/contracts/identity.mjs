import { contentAddressedId, framedHash } from "../domain/hashing.mjs";
...
export function deriveStableId(input) {
  ...
  if (input.kind === "thread") {
    ...
    return {
      id: contentAddressedId("thr_", "codex-threadgraph/thread-id/1", [hostId, nativeThreadId]),
      policyVersion: "id-policy/1-alpha",
    };
  }
  if (input.kind === "project") {
    const canonicalProjectId = requireIdentityField(input.canonicalProjectId, "canonicalProjectId");
    return {
      id: contentAddressedId("prj_", "codex-threadgraph/project-id/1", [hostId, canonicalProjectId]),
      policyVersion: "id-policy/1-alpha",
    };
  }
